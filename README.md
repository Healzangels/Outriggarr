...
1. **Subscriptions** attach one or more video sources to a Sonarr series: YouTube channels or playlists, Vimeo channels or showcases, or archive.org collections. On a schedule the scheduler reads the series' missing, monitored, aired episodes from Sonarr, lists the newest videos from the source (50 by default; a subscription can list deeper, up to 5000, when its episodes sit behind hundreds of newer uploads; playlists are always listed whole), and matches them: overrides first, then an optional title regex, then normalised titles, then upload dates. On the schedule, matches become jobs according to the subscription's automatic-download setting: future episodes only (the default, so a big show doesn't dump its backlog), everything Sonarr wants, or nothing automatic. The preview's Download buttons are always yours: all matches, or just the ones you tick. Unmatched episodes are shown with what each strategy saw, so you can pin a video to an episode with one click.
