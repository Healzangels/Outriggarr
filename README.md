...
1. **Subscriptions** attach a video source (channel or playlist) to a Sonarr series. On a schedule the scheduler reads the series' missing, monitored, aired episodes from Sonarr, lists the newest videos from the source, and matches them: overrides first, then an optional title regex, then normalised titles, then upload dates. Each match becomes a job. Unmatched episodes are shown in the GUI with what each strategy saw, so you can pin a video to an episode with one click.
...
Strategies run in a fixed order and the first one that yields exactly one candidate wins; zero or several fall through:
