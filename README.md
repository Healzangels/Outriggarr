...
1. **Subscriptions** attach one or more video sources to a Sonarr series: YouTube channels or playlists, Vimeo channels or showcases, or archive.org collections. On a schedule the scheduler reads the series' missing, monitored, aired episodes from Sonarr, lists the newest videos from the source (50 by default; a subscription can list deeper, up to 5000, when its episodes sit behind hundreds of newer uploads; playlists are always listed whole), and matches them: overrides first, then an optional title regex, then normalised titles, then upload dates. Each match becomes a job. Unmatched episodes are shown in the GUI with what each strategy saw, so you can pin a video to an episode with one click.
